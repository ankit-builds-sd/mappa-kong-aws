...
export function swm__q_sensorCalc(
  swm__pi_projectCategory: string,
  swm__pi_clubHouseUtilitySensor: boolean,
  swm__pi_supplySideTankerUtilitySensor: boolean,
  swm__pi_supplySideBorewellUtilitySensor: boolean,
  swm__pi_inletsPerStudio: number,
  swm__pi_inletsPerOneBHK: number,
  swm__pi_inletsPerTwoBHK: number,
  swm__pi_inletsPerThreeBHK: number,
  swm__pi_inletsPerFourBHK: number,
  ...
  swm__st_chSensors: number,
  swm__st_borewellSensors: number,
  swm__st_tankerSensors: number,
  swm__st_otherSensors: number
) {
  let swmStudio = null;
  let swmOneBHK = null;
  let swmTwoBHK = null;
  ...
  swmNumberOfSensors = swmStudioSensors + swmOneBHKSensors + swmTwoBHKSensors + swmThreeBHKSensors + swmFourBHKSensors;
  ...
  if (swm__pi_supplySideTankerUtilitySensor === true) {
    swmNumberOfUtilitySensors = swmNumberOfUtilitySensors + swm__st_tankerSensors;
  }
  if (swm__pi_supplySideBorewellUtilitySensor === true) {
    swmNumberOfUtilitySensors = swmNumberOfUtilitySensors + swm__st_borewellSensors;
  }
...
export function swm__q_outcomes(
  swm__pi_capexPreSdPlus: number,
  global__st_convertLtoKL: number,
  global__st_daysInYear: number,
  swm__st_fixedCost: number,
  swm__st_sensorInstallationCost: number,
  swm__q_sensorCalc,
  water__q_currentWaterTable
) {
  let productIdentifier = 'dummy';
  let swmDaily;
  let swmAnnualWater;
  ...
  let swmCapex = null;
  let swmOpex = null;
  swmDaily = (water__q_currentWaterTable.smartWaterMeter.residentialDomestic * 100) / global__st_convertLtoKL;
  swmAnnualWater = swmDaily * global__st_daysInYear;
  swmUnitApartmentCapex = 100;
  swmUnitUtilityCapex = 100;
  swmCapexPostSdPlus =
    (swmUnitApartmentCapex + swm__st_sensorInstallationCost) * swm__q_sensorCalc.swmNumberOfSensors +
    (swmUnitUtilityCapex + swm__st_sensorInstallationCost) * swm__q_sensorCalc.swmNumberOfUtilitySensors +
    swm__st_fixedCost;
...
export function swm__q_outcomesPreSDPlus(project__pi_city: string, global__st_daysInYear: number, swm__st_dailyPreSDPlus: number, swm__q_sensorCalc) {
  let swmAnnualWaterPreSDPlus = swm__st_dailyPreSDPlus * global__st_daysInYear;
...
export function swm__q_projectLevelInsights(
  swm__pi_annualBaseLineWaterConsumption: number,
  global__st_convertKgToTon: number,
  global__st_treesPerTonCF: number,
  swm__q_outcomes
) {
  let swmCFMitigated = null;
  let swmSdgNumber = 100;
  let swmTreesSaved = null;
  let swmPercentageTotalWaterConsumption = null;
  swmCFMitigated = 1.6 * swm__q_outcomes.swmAnnualWater * global__st_convertKgToTon;
  swmTreesSaved = swmCFMitigated * global__st_treesPerTonCF;
  swmPercentageTotalWaterConsumption = (swm__q_outcomes.swmAnnualWater * 100) / swm__pi_annualBaseLineWaterConsumption;
  return { swmCFMitigated, swmTreesSaved, swmPercentageTotalWaterConsumption };
}
export function swm__q_flowDiagram(swm__q_outcomes) {
  let swmFlowDiagramDomestic = swm__q_outcomes.swmDaily;
...
export function swm__q_passiveEnergy(
  project__pi_numberOfFloors: number,
  project__pi_floorToFloorHeight: number,
  global__st_convertJoulesTokW: number,
  global__st_convertLtoM3: number,
  global__st_accelarationDueToGravity: number,
  water__st_pumpEfficiency: number,
  water__st_motorEfficiency: number,
  water__st_specificGravityOfWater: number,
  water__st_pumpFlowRate: number,
  swm__q_outcomes
) {
  ...
  timeTakenToPumpWater = (swm__q_outcomes.swmAnnualWater * global__st_convertLtoM3) / water__st_pumpFlowRate;
  buildingHeight = project__pi_numberOfFloors * project__pi_floorToFloorHeight;
  hydraulicPower =
    (water__st_pumpFlowRate * buildingHeight * global__st_accelarationDueToGravity * water__st_specificGravityOfWater) / global__st_convertJoulesTokW;
...
export function swm__q_secondaryImpact(swm__q_passiveEnergy) {
  let passiveEnergykWh = null;
...
export function swm__q_goalMonitoring(swm__q_outcomes) {
  let waterGoal = null;
...
export function swm__q_overviewBarGraph(swm__q_outcomes) {
  let title = null;
...
export function swm__q_overviewBarDonutGraph(swm__q_outcomes) {
  let donutOneTitle = null;
